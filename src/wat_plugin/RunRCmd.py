...
def callR(scriptFile, opts, args=[], relativeScript=True, relativeR=True, shell=False):
	...
	rDirectory = System.getProperty("R.Directory")
	rVersion = System.getProperty("R.Version")
	rLibrary = System.getProperty("R.Library")
	RScriptExe = os.path.join(rDirectory, rVersion, "bin", "Rscript.exe")
	...
	# assemble command
	cmdLine = []
	cmdLine += [RScriptExe, "--vanilla", stringWrap(scriptFile), stringWrap(os.path.join(rDirectory, rVersion))] + args
